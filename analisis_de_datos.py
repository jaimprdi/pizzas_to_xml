...
def informacion_a_xml(fichero,dataframe, guardar):
    ...
    for j in range(len(columnas)):
        ...
        et.SubElement(columna,'NaN',{'_NaN_columna':str(df[columnas[j]].isna().sum())}) #nans
        et.SubElement(columna,'null',{'_null_columna':str(df[columnas[j]].isnull().sum())}) #nulls
        et.SubElement(columna,'tipo_dato',{'tipo_columna':str(df[columnas[j]].dtype)}) #categoria de columns (df.types)
